...
import {
  ...
  Code, Settings, Bell, Search, Activity, TrendingUp, RefreshCw, Map, Menu
} from "lucide-react";
...
const AdminPage = () => {
  ...
  return (
    <div className="min-h-screen flex bg-background">
      ...
      {/* Sidebar */}
      <aside className={`
        fixed lg:static inset-y-0 left-0 z-50 w-64 min-h-screen flex flex-col bg-sidebar text-sidebar-foreground flex-shrink-0
      ...
      `}>
        <div className="p-6 flex items-center gap-3">
          ...
          <button onClick={() => setSidebarOpen(false)} className="ml-auto lg:hidden p-1.5 rounded-lg text-sidebar-foreground/70 hover:text-white">
            <X size={20} />
          </button>
        </div>
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="relative hidden sm:block">
                ...
                <input
                  ...
                  className="pl-9 pr-4 py-2 rounded-lg bg-muted border-0 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring w-40 md:w-64"
                />
              </div>
        ...
        <div className="p-4 sm:p-6 lg:p-8">
          {loading ? (
            ...
            <>
              ...
              {tab === "dashboard" && (
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                    {statCards.map((s) => (
                      <div key={s.label} className="p-4 sm:p-5 rounded-2xl bg-card shadow-card border border-border">
                        <div className="flex items-center justify-between mb-3">
                          <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center ${s.color}`}>
                            <s.icon size={18} />
                          </div>
                        ...
                        <p className="text-xl sm:text-2xl font-bold text-foreground mt-1 tabular-nums">{s.value.toLocaleString()}</p>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
                    <div className="lg:col-span-3 rounded-2xl bg-card shadow-card border border-border p-4 sm:p-5">
                    ...
                    <div className="lg:col-span-2 rounded-2xl bg-card shadow-card border border-border p-4 sm:p-5">
                      <h3 className="font-bold text-foreground mb-3">Mapa de Alertas Ativos</h3>
                      <div className="h-48 sm:h-64 rounded-xl bg-muted flex items-center justify-center">
                  ...
                  {/* API Status */}
                  <div className="rounded-2xl bg-card shadow-card border border-border p-4 sm:p-5">
              ...
              {tab === "usuarios" && (
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <h1 className="text-xl sm:text-2xl font-bold text-foreground">Usuárias</h1>
                      ...
                      <table className="w-full text-sm min-w-[600px]">
                        <thead>
                          <tr className="border-b border-border bg-muted/50">
                            <th className="text-left px-4 sm:px-5 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Usuária</th>
                            <th className="text-left px-4 sm:px-5 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Status</th>
                            <th className="text-left px-4 sm:px-5 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Localização</th>
                            <th className="text-left px-4 sm:px-5 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Cadastro</th>
                            <th className="text-right px-4 sm:px-5 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Ações</th>
                          </tr>
                        </thead>
                        <tbody>
                          {usuarios.map((u: any) => (
                            <tr key={u.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                              <td className="px-4 sm:px-5 py-4">
                                <div className="flex items-center gap-3">
                                  <div className="w-9 h-9 rounded-full bg-primary/10 flex items-center justify-center overflow-hidden text-xs font-bold text-primary flex-shrink-0">
                              ...
                              <td className="px-4 sm:px-5 py-4 text-muted-foreground">{u.cidade ? `${u.cidade}/${u.estado}` : "—"}</td>
                              <td className="px-4 sm:px-5 py-4 text-muted-foreground">{formatDate(u.created_at)}</td>
                              <td className="px-4 sm:px-5 py-4">
              ...
              {tab === "alertas" && (
                <div className="space-y-4">
                  <h1 className="text-xl sm:text-2xl font-bold text-foreground">Alertas</h1>
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm min-w-[500px]">
                        <thead>
                          <tr className="border-b border-border bg-muted/50">
                            <th className="text-left px-4 sm:px-5 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Status</th>
                            <th className="text-left px-4 sm:px-5 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Tipo</th>
                            <th className="text-left px-4 sm:px-5 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Localização</th>
                            <th className="text-left px-4 sm:px-5 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Data</th>
                            <th className="text-right px-4 sm:px-5 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Ações</th>
                          </tr>
                        </thead>
                        <tbody>
                          {alertas.map((a: any) => (
                            <tr key={a.id} className="border-b border-border last:border-0 hover:bg-muted/30">
                              ...
                              <td className="px-4 sm:px-5 py-4 text-foreground">{a.tipo_alerta}</td>
                              <td className="px-4 sm:px-5 py-4 text-muted-foreground">
                                {a.latitude ? `${a.latitude.toFixed(4)}, ${a.longitude.toFixed(4)}` : "—"}
                              </td>
                              <td className="px-4 sm:px-5 py-4 text-muted-foreground">{formatDate(a.created_at)}</td>
              ...
              {tab === "incidentes" && (
                <div className="space-y-4">
                  <h1 className="text-xl sm:text-2xl font-bold text-foreground">Incidentes</h1>
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm min-w-[500px]">
                        <thead>
                          <tr className="border-b border-border bg-muted/50">
                            <th className="text-left px-4 sm:px-5 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Tipo</th>
                            <th className="text-left px-4 sm:px-5 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Descrição</th>
                            <th className="text-left px-4 sm:px-5 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Localização</th>
                            <th className="text-left px-4 sm:px-5 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Data</th>
                            <th className="text-right px-4 sm:px-5 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Ações</th>
                          </tr>
                        ...
                        <tbody>
                          {incidentes.map((i: any) => (
                            <tr key={i.id} className="border-b border-border last:border-0 hover:bg-muted/30">
                              <td className="px-4 sm:px-5 py-4 font-medium text-foreground">{i.tipo}</td>
                              <td className="px-4 sm:px-5 py-4 text-muted-foreground max-w-xs truncate">{i.descricao || "—"}</td>
                              <td className="px-4 sm:px-5 py-4 text-muted-foreground">{i.latitude.toFixed(4)}, {i.longitude.toFixed(4)}</td>
                              <td className="px-4 sm:px-5 py-4 text-muted-foreground">{formatDate(i.created_at)}</td>
                              <td className="px-4 sm:px-5 py-4 text-right">
              ...
              {tab === "planos" && (
                <div className="space-y-4">
                  <h1 className="text-xl sm:text-2xl font-bold text-foreground">Planos de Assinatura</h1>
                  <PlanosTab />
                </div>
              )}
              ...
              {tab === "apis" && (
                <div className="space-y-4">
                  <h1 className="text-xl sm:text-2xl font-bold text-foreground">Gerenciar APIs</h1>
                  <ApiKeysTab />
                </div>
              )}
              ...
              {tab === "mapa" && (
                <div className="space-y-4">
                  <h1 className="text-xl sm:text-2xl font-bold text-foreground">Mapa em Tempo Real</h1>
              ...
              {tab === "config" && (
                <div className="space-y-6 max-w-xl">
                  ...
                  <div className="p-4 sm:p-6 rounded-2xl bg-card shadow-card border border-border space-y-4">
                    <div className="flex items-center gap-3">
        ...
        {editingUser && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-md bg-card rounded-2xl p-5 sm:p-6 space-y-4 shadow-elevated border border-border"
            >
              <div className="flex items-center justify-between">
